...
struct matrix mul(struct matrix x,struct matrix y,int size){//矩阵乘法 
	struct matrix tmp;
...
int main()
{
    struct matrix a,b;
    for(int i=0;i<2;i++)
    for(int j=0;j<2;j++)a.mat[i][j]=j+1;
    for(int i=0;i<2;i++)
    for(int j=0;j<2;j++)b.mat[i][j]=0;
    matrix_pow(a,&b,2,2);
    for(int i=0;i<2;i++)
    for(int j=0;j<2;j++)printf(j==1?"%d\n":"%d ",b.mat[i][j]);
