...
public class PageBreakPrintMode {
    ArrayList<Dish> dishes;
    private final int maxPerCnt;//每页的显示个数
    private int pageNow;//当前显示页
    private final int countPage;//总页数
    private int count = 0;
    
    public PageBreakPrintMode(ArrayList<Dish> dishes, int startPage, int maxPerCnt) {
        this.dishes = dishes;
    ...
    public boolean parseCommand(String line) {
        if (count <= 0) {
            System.out.println("entity.Menu is empty, exit page check mode");
            return false;
        }
