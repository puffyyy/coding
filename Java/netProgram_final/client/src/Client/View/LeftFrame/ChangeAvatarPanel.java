...
public class ChangeAvatarPanel extends JPanel {
    ...
    private MyScrollBarUI.MyButton openButton;
    private JPanel contentPanel;
    private File selectedFile;
    private JLabel statusLabel;
    
    private final int imageMaxWidth = 350;
    private final int imageMaxHeight = 200;
    ...
    private void initComponents() {
        User currentUser = ClientCache.currentUser;
        Image avatar = new ImageIcon(AvatarUtil.createOrLoadUserAvatar(currentUser).getScaledInstance(200, 200, Image.SCALE_SMOOTH)).getImage();
        ...
        okButton = new MyScrollBarUI.MyButton("使用头像", Colors.MAIN_COLOR, Colors.MAIN_COLOR_DARKER, Colors.MAIN_COLOR_DARKER);
        okButton.setPreferredSize(new Dimension(100, 35));
        
        openButton = new MyScrollBarUI.MyButton("选择图片", Colors.MAIN_COLOR, Colors.MAIN_COLOR_DARKER, Colors.MAIN_COLOR_DARKER);
            ...
            public void mouseClicked(MouseEvent e) {
                if (okButton.isEnabled()) {
                    ...
                    if (selectedFile == null) {
                        JOptionPane.showMessageDialog(MainFrame.getContext(), "请选择新的头像", "选择图片", JOptionPane.WARNING_MESSAGE);
                    ...
                    if (selectedImage == null) {
                        ...
                        JOptionPane.showMessageDialog(MainFrame.getContext(), "更改头像", "是否更改头像", JOptionPane.INFORMATION_MESSAGE);
                        AvatarUtil.saveUserAvatar(selectedImage, ClientCache.currentUser.getUid());
                        MyInfoPanel.getContext().reloadAvatar();
                        AvatarUtil.deleteDefaultUserAvatar(ClientCache.currentUser.getUid());
                        showSuccessMessage();
                        restoreOKButton();
                        ClientCache.currentUser.setAvatar(selectedImage);
                        Request request = new Request(RequestType.CHANGE_AVATAR);
...
class ImageAdjustLabel extends JLabel {
    ...
    /**
     * 图象调整后的头像Panel构造方法
     * @param imageMaxWidth 图象最大宽度
     * @param imageMaxHeight 图象最大高度
     * @param initAvatar 初始头像
     */
