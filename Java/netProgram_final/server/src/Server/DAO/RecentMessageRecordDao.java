    ...
    public static ArrayList<Message> groupMessage(Long gid) {
        ...
        try {
            statement = con.prepareStatement(
                    "select m_send_uid,m_group,message_text,m_time from message where m_group=? group by m_send_uid,message_text,m_group,m_time order by m_time"
            );
...
//                        res.getInt(7), res.getBoolean(8));
                Message message = new Message(res.getLong(1), gid, res.getString(3)
